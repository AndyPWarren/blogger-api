...
def post_message(connection, url, success, project):
  ...
  project_url = "https://bitbucket.org/spectrakey/" + project
  branch_url = "https://bitbucket.org/spectrakey/" + project + "/branch/" + branch

  status_text = 'succeeded' if success else 'failed'
  color = 'good' if success else 'danger'
  text = '%s\'s <%s|build #%s> %s: push in <%s|%s> (<%s|%s>)' % (author, build_url, build_number, status_text, project_url, project, branch_url, branch)
